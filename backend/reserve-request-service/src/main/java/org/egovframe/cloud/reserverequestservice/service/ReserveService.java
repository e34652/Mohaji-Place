        ...
        return create(saveRequestDto)
                .flatMap(reserveResponseDto ->
                        Mono.fromCallable(() -> {
                            ...
                            log.info("Binding successfully created");

                            // 메시지 전송
                            boolean sent = streamBridge.send("reserveRequest-out-0", reserveResponseDto);
                            if (sent) {
                                log.info("Message successfully sent to streamBridge");
                            } else {
                                log.error("Failed to send message to streamBridge");
                            }
